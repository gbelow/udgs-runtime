import type { Action, CombatState, MoveAction } from '../types'
...
// blast instead (`getMoveAfter`).
export function getMoveBeforeBlast(reaction: Action): ReactionMove | null {
  if (reaction.kind !== 'avoidExplosion' || !reaction.roll) return null
...
// The move a reaction opens once the action it answers has landed.
...
// cancelled").
export function getMoveAfter(state: CombatState, root: Action, reaction: Action): ReactionMove | null {
