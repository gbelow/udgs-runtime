...
describe('used abilities', () => {
  ...
  it.each(actives)('"%s" pays its price and instant drain at the use and its round-change drain once, at the next round', (key) => {
    ...
    const round = (c: CampaignCharacter) => nextRound({ characters: { c }, activeCharacterId: 'c', round: 0, inTurnCharacter: 'c', actions: [], stack: [], board: null, grapples: [], floor: [] }).characters.c
