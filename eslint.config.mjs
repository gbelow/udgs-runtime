  ...
  //    for a type) inverts that arrow and lets Zustand define the domain's shape.
  {
    ...
    rules: {
      "no-restricted-imports": ["error", {
        patterns: [
          {
            group: ["**/stores/*", "**/stores/**", "react", "react-dom", "zustand", "zustand/**"],
            message:
              "The domain layer is pure and React-free. Data flows domain -> stores -> hooks -> UI, so the domain must define its own types and never import from app/stores or React. If a store needs a shape, declare it in the domain and have the store import it.",
          },
        ],
      }],
    },
